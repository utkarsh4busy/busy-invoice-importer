...
def call_ocr_api(file_bytes: bytes, filename: str, token: str):
    """Returns (response_json, request_meta). request_meta has sanitized info safe to display."""
    headers = {
        "Source": "web",
        "Service": "ocr",
    ...
    # Build request metadata for display (never expose full token)
...
def determine_gst_type(seller_gstin: str, place_of_supply: str) -> str:
    """Infer IGST vs CGST/SGST from seller state and place of supply."""
...
def build_party_address(seller: dict, buyer: dict) -> str:
    ...
    if addr:
        parts.append(addr)
    # append buyer info in the style of the example
...
def json_to_rows(api_json: dict) -> list[dict]:
    ...
    # reformat date dd-mm-yyyy → dd-mm-yyyy (already that format, keep)
    ...
    for item in line_items:
        ...
        # TCS/TDS ~ 2% of taxable rounded to int (matches example pattern)
...
with st.sidebar:
    ...
    st.markdown("1. Upload one or more PDFs\n2. Each PDF is sent to the OCR API\n3. Responses are mapped to Busy import format\n4. Download the filled Excel")

# ─── TABS ─────────────────────────────────────────────────────────────────────
tab_upload, tab_results = st.tabs(["📤 Upload & Process", "📊 Results & Download"])

# ─── SESSION STATE ────────────────────────────────────────────────────────────
if "results" not in st.session_state:
    st.session_state.results = []   # list of {filename, status, rows, error, raw_json}
    ...
    if uploaded_files:
        ...
        if process_btn:
            ...
            for i, pdf_file in enumerate(uploaded_files):
                ...
                result = {"filename": pdf_file.name, "status": None, "rows": [], "error": "", "raw_json": None,
                          "ocr_time": None, "excel_time": None, "request_meta": None}
                ...
                try:
                    ...
                    if raw.get("status"):
                        ...
                        with log_area:
                            ocr_str = f'<span class="timing-pill ocr">🔍 OCR: {result["ocr_time"]:.2f}s</span>' if result["ocr_time"] is not None else ""
                            st.markdown(
...
with tab_results:
    if not st.session_state.results:
        ...
        for r in results:
            ...
            with st.expander(f"{status_icon} {r['filename']} — {len(r['rows'])} rows"):
                # Timing summary
                ...
                if r.get("request_meta") or r.get("raw_json"):
                    ...
                    with api_req_tab:
                        if r.get("request_meta"):
                            ...
                            st.info("Request metadata not available (error before API call).")
...
# ─── COLUMN SCHEMA (backend reference only — not rendered in UI) ───────────────
# Maps each Excel output column to its API source field and transformation logic.
# Update this when field mappings change; intentionally not exposed in the UI.
COLUMN_SCHEMA = [
    ("Invoice_Type",          "Hardcoded",                      '"Purchase"'),
    ("Voucher_Date",          "invoice_date",                   "Parsed from API"),
    ("Voucher_No",            "—",                              "Left blank (assigned by Busy)"),
    ("Supplier_Invoice_No",   "invoice_number",                 "e.g. KMIT/331/2024-25"),
    ("Supplier_Invoice_Date", "invoice_date",                   "Same as invoice date"),
    ("Party_Name",            "seller.name",                    "Seller company name"),
    ("Party_GSTIN",           "seller.gstin",                   "15-char GSTIN"),
    ("Party_PAN",             "seller.gstin[2:12]",             "Extracted from GSTIN"),
    ("Party_Address",         "seller.address + buyer details", "Combined address block"),
    ("Place_of_Supply",       "place_of_supply",                "e.g. Delhi (07)"),
    ("Reverse_Charge",        "reverse_charge",                 '"Y" or "N"'),
    ("Invoice_Category",      "Derived",                        '"Services" for SAC 998313'),
    ("Item_HSN_SAC",          "line_items[].hsn_sac",           ""),
    ("Item_Description",      "line_items[].description",       ""),
    ("Quantity",              "line_items[].quantity",          ""),
    ("Unit",                  "line_items[].unit",              ""),
    ("Rate_per_Unit",         "line_items[].rate",              ""),
    ("Discount_Pct",          "line_items[].discount",          "0 if null"),
    ("Taxable_Amount",        "rate x quantity",                "Computed"),
    ("IGST_Pct / Amount",     "Derived from GSTIN",             "Inter-state -> IGST"),
    ("CGST_Pct / Amount",     "Derived from GSTIN",             "Intra-state -> CGST+SGST"),
    ("SGST_Pct / Amount",     "Derived from GSTIN",             "Intra-state -> CGST+SGST"),
    ("TCS_TDS_Amount",        "Estimated ~2% of taxable",       ""),
    ("Invoice_Total",         "summary.grand_total",            ""),
    ("Narration",             "invoice_number + invoice_date",  "e.g. Inv No: X Dt: Y"),
]